...
- Markdown 正文图片支持点击放大查看，避免仪表盘和图示在正文宽度内看不清。
- 详情页左侧“学到的知识”从标签堆叠升级为学习地图列表，显示数量、顺序和清楚的阅读路线。
- 每篇最终复盘 Markdown 在 frontmatter 后新增 `TLDR`，用分点方式沉淀关键词、表达方法、关键知识、决策、避坑和下次行动，方便快速学习。
...
11. AI 按用户选择进行调研和写作。
12. AI 在最终 Markdown 顶部写入 `TLDR`：至少包含关键词、表达方法、关键知识、避坑或下次行动中的 5 条以上分点内容。
...
| 当前展开任务区 | 会话背景、用户原话、表达待优化原话、知识区卡片、反馈框 | 勾选、选择深度、提交 |
...
- Given 历史复盘 Markdown 中包含仪表盘、流程图或截图图片
- When 用户点击正文中的图片
...
- Then frontmatter 后第一个正文区块必须是 `## TLDR`，并用分点陈述值得记住的关键词、表达方法、知识、决策、避坑或行动
...
- [ ] 最终复盘 Markdown 的 frontmatter 后必须出现 `## TLDR`。
- [ ] TLDR 至少 5 条，且至少包含 1 条“下次可以说：...”表达方法。
...
- [ ] TLDR 使用短分点，不写成大段摘要；用户只看 TLDR 也能带走本次最值得记住的内容。
