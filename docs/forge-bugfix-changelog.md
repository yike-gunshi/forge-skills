...
v7.0/v7.1 已经让单 bug 具备完整报告、QA 证据和用户闭环，但在“Pass + 新发现”场景里仍有一个隐性风险：AI 容易把新问题直接写入 backlog，甚至顺着继续修，导致用户感知不到“原 bug 已经完成”的边界。

### 核心改变

- **新增 Pass 边界确认**
  - 原 bug 一旦通过 QA / 人工验收 / 用户问题闭环断言，AI 必须先告知“本次 bug 修复已完成”
  - 不论新问题来自用户反馈、forge-qa、批次验收，还是 AI 自己发现的衍生问题，都不得直接继续修
- **新问题入档需要用户确认**
  - 用户确认后才写入 `docs/bugfix/backlog.md` 或报告索引
  - 用户不确认时，不默默入档，也不把新问题当作原 bug 的 Fail
- **确认入档后给简短接力 prompt**
  - 接力 prompt 只说清：原 bug 已完成、报告路径、新登记编号、建议新会话重新判断范围和根因
  - 不把完整根因、diff、方案塞给下个会话，保留新会话自己的判断空间
- **修复中发现的衍生问题改为待确认**
  - 原 bug 未 Pass 前，新问题只列为“待确认新发现”
  - 原 bug Pass 后统一进入 P7.4 询问，再决定是否 P7.5 分流

### 新增/修改清单

| 文件 | 性质 | 说明 |
|---|---|---|
| `skills/forge-bugfix/SKILL.md` | 改 | 新增 P7.4 Pass 边界确认；调整 P5.2 / P7.3 / P7.5 / 铁律 |
| `skills/forge-bugfix/templates/review-checklist.md` | 改 | PASS + 新发现改为“先确认入档，再分流 backlog” |
| `docs/forge-bugfix-changelog.md` | 改 | 增加 v7.2 演进记录 |

### 设计决策

- **为什么不自动写 backlog**：新问题可能只是用户随口补充、需求表达或尚未复现的观察。入档会影响后续优先级，应该让用户确认。
- **为什么先说 bug 已完成**：这是防止 bugfix 会话失控的封闭节点。用户可以继续推进新问题，但心理模型必须从“这个 bug 没修好”切到“下一个问题”。
- **为什么 prompt 要短**：接力 prompt 是给新会话启动用的，不是替新会话做完判断。它只交代入口，不预设结论。
