...
- 流程、因果链、状态机等结构图优先用 Mermaid / show-widget；`forge-fupan` 的知识模块学习图使用 Image 2。
...
- `/forge-fupan`：每个展开知识模块生成一张 Image 2 学习图，最终复盘仍以文本诊断和知识沉淀为主。
...
- 每个最终展开知识模块会生成 1 张学习图；图片失败时保留 prompt pack，不阻塞复盘。
