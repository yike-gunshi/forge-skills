...
- `QA.md` 当前验收手册或本次 QA 报告区
- 结构化 Bug 候选（分级 + 步骤 + 截图），交给 forge-bugfix 创建活跃 BF 报告
- 历史 QA 结论按项目规则进入 changelog 或 archive，不把 `QA-CHANGELOG.md` 当所有项目必备文件
...
- 每个 bug 独立活跃报告 `docs/bugfix/reviews/BF-XX.md`，`status: draft`
- 每个 bug 独立 worktree、独立 TDD、独立 commit、独立 QA 回归
- `docs/bugfix/backlog.md` 做任务池和状态看板
- 用户验收 + merge 后，报告移动到 `docs/archive/raw/bugfix-reviews/` 并在 backlog 保留链接
...
- 流程、因果链、状态机等结构图优先用 Mermaid / show-widget；`forge-fupan` 的知识模块学习图默认强尝试使用 Image 2，失败时保留 prompt pack。
...
- `/forge-prd`：在 `.features/{feature-id}/feature-spec.md` 中记录视觉决策需求和已有图链接。
...
- `/forge-fupan`：每个展开知识模块强尝试生成一张 Image 2 学习图，最终复盘仍以文本诊断和知识沉淀为主；最终回复必须报告已生成/待生成数量。
...
**命令**：`/forge-fupan`
**触发词**：「复盘」「总结知识」
**职责**：提取 Claude/Codex 会话背景 → 打开本地 Workbench 确认学习地图和补充反馈 → 按用户选择调研 → 为每个知识模块生成学习图 → 结构化复盘
...
- 本地服务默认只监听 `127.0.0.1`。
- 首页展示待确认复盘任务和历史复盘。
- 用户在页面中选择知识点数量和学习深度：`了解 / 表达 / 复现`。
- 用户填写的补充反馈会作为后续调研和最终输出约束，不只是备注。
- 每个最终展开知识模块会强尝试生成 1 张学习图；图片失败时保留 prompt pack，不阻塞复盘，并在最终回复说明已生成 N 张 / 待生成 N 张。
