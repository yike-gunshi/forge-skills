...
### D-01 forge-qa 瘦身（64KB → 骨架约 12-15KB）
...
### D-02 forge-bugfix 瘦身（63KB → 骨架约 15KB）
...
### D-03 中型 skill 瘦身（design 30K / dev 28K / eng 28K / brainstorm 27.5K / prd 26K）
...
### D-04 15 个 description 压缩到 2-4 行
...
### D-05 工作区 CLAUDE.md 的 forge 段收缩（约 200 行 → 约 60 行）
...
### D-06 兑现 init-project.sh → 新项目一键接入
...
### D-08 dev/deliver 合并（方向已按你的答复定：保 dev）
...
### D-10 forge-qa 引擎清单去掉 gstack/browse（P-31）
