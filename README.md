...
同一思路贯穿全体系：每个 SKILL.md 只保留骨架和铁律，阶段细则拆进 `references/` 按需加载（如 forge-bugfix 63KB → 12.6KB 骨架 + 4 份阶段手册）。2026-07-17 对标 Anthropic skill-creator 与 yao-meta-skill 完成整改后，15 个 SKILL.md 总量再降 12.5%，全部远离官方 500 行红线；触发路由用 `evals/trigger-cases.md` 回归。
