...
`skills/_shared/` 存放被多个 Skill 引用的公共规范（当前文档加载协议、视觉决策层），随 install.sh 一起 symlink。视觉决策层统一约定：结构问题优先 Mermaid，页面气质判断用 Image 2 视觉稿，最终验收只认真实截图 + CSS 断言 + Feature Spec；复盘默认不配图，用户点名才生成学习图。
...
同一思路贯穿全体系：每个 SKILL.md 只保留骨架和铁律，阶段细则拆进 `references/` 按需加载（如 forge-bugfix 63KB → 12.6KB 骨架 + 4 份阶段手册）。
