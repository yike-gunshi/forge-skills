...
**纯验收模式：测试 + 报告，不修代码。** 发现的问题生成修复清单或回填验收清单。
...
| 模式 | 触发条件 | 输入 | 输出 | 下游 |
|---|---|---|---|---|
| **Mode A：完整 QA** | 用户直接触发，或 forge-dev 调度 | PRD / DESIGN / git diff | QA.md 报告 + User Gate | forge-ship / forge-eng |
| **Mode B：单 bug 验收清单** | forge-bugfix 的 P6 调用；入口带参数 `review_doc=docs/bugfix/reviews/BF-XX.md` | 单 bug 验收清单文档 | 清单内 "QA 验证" 列回填 + "QA 验收记录"节填充 | forge-bugfix 的 P6.5（用户人工验收）或 P5（回修）|
...
Mode B 详见"## Mode B：单 bug 验收清单模式"节（本文档末尾）。
...
|---|---|---|
| `mode=B` | ✅ | 强制信号，优先级最高 |
| `review_doc=<路径>` | ✅ | 验收清单文档（存在性校验失败直接 exit） |
...
> 🎯 配合 forge-bugfix 的 P6 调用。目标：针对**一个 bug 的验收清单**跑自动化测试，把结果回填到同一份清单文档里。

### B.1 前提与入口

- **触发方**：forge-bugfix 的 P6 节点
- **传入参数**：`REVIEW_DOC`（验收清单文档路径，形如 `docs/bugfix/reviews/BF-0419-2.md`）
...
- 涉及文件列表（用于缩小测试范围）
- 验证项表格的每一行（验证项 / 预期效果 / 操作步骤）

### B.3 为每个验证项选择测试引擎

| 验证项性质 | 默认引擎 | 选择理由 |
|---|---|---|
| UI 交互 / 视觉 / 控制台 | gstack/browse 或 Playwright | 需要真实浏览器 |
...
3. **回填** "QA 验证" 列：
   - ✅ 通过
   - ❌ 未通过（附一句话原因 + 证据链接）
4. 截图保存到 `.gstack/bugfix-reports/screenshots/${BUG_ID}-qa-N.png`
...
除了验证项表格的"QA 验证"列，AI 还要填充清单文档里"## QA 验收记录"这一节：
...
**自动化测试范围**：
- 跑了 gstack/browse 重放（3 步：打开登录 / 登录 / 查看头像）
...
| # | 状态 | 证据 |
|---|---|---|
| 1 | ✅ | screenshots/BF-0419-2-qa-1.png |
| 2 | ✅ | screenshots/BF-0419-2-qa-2.png |
| 3 | ❌ | 控制台报 `TypeError: Cannot read property 'avatar'`。日志: .gstack/bugfix-reports/BF-0419-2-console.log |
...
- **所有验证项 ✅**：
  ```
  ✅ QA_PASS (BF-0419-2)
  清单已回填：docs/bugfix/reviews/BF-0419-2.md
  下一步：交还 forge-bugfix，进入 P6.5 用户人工验收。
  ```
...
- **至少一条 ❌**：
  ...
  清单已回填 ❌ + 证据。
  下一步：交还 forge-bugfix，回 P5 继续修复（不打扰用户）。
  ```
...
- ❌ 不主动判断"修复是否正确"（只按清单项断言，是否通过由用户在 P6.5 决定）
