# forge-qa · Mode A 执行手册（第0步~第8步全细则）

> 由 SKILL.md 骨架按需加载：执行 Mode A 完整 QA 时必读本文件。
> 铁律、调用契约、流程总览在 SKILL.md；10 维度代码模板在 test-dimensions.md；报告模板在 qa-template.md。

## 导航（按需跳读）

> **⚡ 本文件 1000+ 行，不要整读。** 按当前执行到的步骤，搜小节标题跳读对应节即可。

| 小节 | 何时需要读 |
|---|---|
| 第0步：上下文探测与环境准备 | 每次 Mode A 启动时必读：worktree 检测、文档链定位、变更范围分析、选择器审计、测试级别选择 |
| 第1步：建立健康基准 | 开始测试前，先跑一遍应用确认基线健康状态时 |
| 第2步：理解现状 | 判断走迭代模式（已有 QA.md）还是从零创建模式时 |
| 第2.5步：生成 test-spec | 编写测试前必读：test-spec.json 结构、断言深度规则（铁律 4） |
| 第3步：生成验收计划并请用户确认 | 需要产出验收计划文档并等用户确认时 |
| 第4步：更新 QA 文档 | 用户确认计划后、执行测试前的文档落盘时 |
| 第5步：10 维度测试执行 | 实际编写与运行测试脚本时：脚本规范、分阶段执行、浏览器/纯代码引擎选择 |
| 第6步：智能分析 + Bug 报告 | 测试跑完后：Bug 登记格式、严重度分类、修复清单、交付 forge-bugfix |
| 第7步：User Gate（用户验收关卡） | 报告产出后等用户验收时：举一反三机制、FEEDBACK.md 结构与闭环 |
| 第8步：健康评分与报告 | 产出最终报告时：评分计算、报告结构、终端报告 |
...
**文档版本校验**：读取文档后提取版本号，与 `.features/{feature-id}/status.md` 中记录的 PRD 版本对比。不一致则警告。
...
使用 Playwright（或 browser-use）截取基准截图和控制台状态。
...
优先读取 `.features/{feature-id}/feature-spec.md`。如果存在：
...
如果 Feature Spec 不存在：
- legacy 项目可兼容读取 PRD 内嵌 Feature Spec，但应在 QA 报告中建议迁移到 `.features`
- 通过 AskUserQuestion 警告：「没有找到 `.features` Feature Spec，QA 将基于 PRD 功能描述生成测试，但验收标准可能不够精确。建议先运行 /forge-prd 补充 Feature Spec。」
...
| # | 验收项 | 来源 | 测试方法 | 断言类型 |
|---|--------|------|---------|---------|
| 1 | {场景描述} | Feature Spec: {Requirement名}.正常 | {Playwright/browser-use} | {contains_text/css_value/...} |
...
1. 更新/创建 `docs/QA.md` 当前验收手册或本次 QA 报告区（参考 [references/qa-template.md](references/qa-template.md)）
2. 若本次改变了长期验收规则，把有效结论写入 `docs/CHANGELOG.md` 或项目指定 QA 历史入口；不要假设所有项目都有 `QA-CHANGELOG.md`
...
**建议严重度**：P0 / P1 / P2
**是否属于本轮范围**：是 / 否 / 待用户判断
**关联 Feature Spec**：.features/{feature-id}/feature-spec.md#...
**用户可见现象**：...
**复现步骤**：
1. ...
2. ...
...
**A) 验收通过（accept）**
- 更新 `.features/{feature-id}/status.md` qa 行为 `[✅ 已完成]`
...
3. **回查 Feature Spec**：
   - 读取 `.features/{feature-id}/feature-spec.md`，检查其他行为场景是否可能存在同类问题
