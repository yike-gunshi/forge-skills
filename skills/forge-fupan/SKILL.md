...
- **复发 ≥3 次触发强制固化**：禁止只再写一条账本条目。本次复盘内必须完成固化动作——
  改对应 skill 的检查项/门禁，或产出让用户亲手改的配置清单（如 settings.local.json allow 规则），
  并在新条目 evidence 里写明固化落点；固化需要用户亲手执行时，把待办明确交给用户，不算完成前不结案。
...
5. **复发必须标注**，不许假装第一次见。复发 ≥2 次 → 建议固化为流程约束；复发 ≥3 次 → 强制固化（见第4步），账本不是回收站。
