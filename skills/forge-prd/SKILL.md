---
name: forge-prd
description: '产品诊断与 PRD 迭代管理器。用户描述遇到的问题或需求，skill 诊断根因（产品设计缺陷/实现偏离/PRD遗漏）、审查模块健康度、设计方案、挑战假设、必要时反驳需求，最终更新 PRD 和 CHANGELOG。支持从零创建 PRD（深度阅读代码+多轮交互）和迭代已有 PRD。触发方式：用户说"更新PRD"、"调整需求"、"迭代PRD"、"forge-prd"、描述产品问题、需要修改项目产品需求时使用。'
---
...
**前提：用户已在第3步明确确认变更清单。** 确认后一次性产出并写入以下内容：
...
4. **更新数据模型**：如涉及表结构或 API 变更
5. **更新已知问题/改进计划**：移除已解决的、新增发现的
6. **更新技术约束**：如有新约束
7. **更新交互流程图**：如交互有变
8. **更新附录修改清单**：按优先级列出所有需执行的修改
9. **自洽性检查**：确保 PRD 内各章节之间不矛盾
