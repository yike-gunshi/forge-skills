...
| `type` | `prd` \| `design` \| `engineering` \| `qa` \| `brainstorm` \| `bugfix` \| `research` \| `plan` \| `guide` \| `reference` \| `changelog` | 文档类型（用于 INDEX 聚合） |
...
| 值 | 含义 | 何时用 |
|---|---|---|
| `live` | 当前权威，AI 应以本文档为准 | 根级当前真相源、当前模块附录、当前 ops runbook |
| `draft` | 草稿 / 未确认 / 进行中 | active feature spec、正在处理的 bugfix review、未确认计划 |
| `archive` | 历史归档，不再更新 | raw archive、旧版本设计稿、已结案 bugfix review |
...
## type 字段的取值清单（11 类，封闭枚举）

| 值 | 对应文档 | 示例 |
|---|---|---|
| `prd` | 产品需求 | `docs/PRD.md` / `.features/{feature-id}/feature-spec.md` |
| `design` | 设计规范 | `docs/DESIGN.md` |
| `engineering` | 工程架构 | `docs/ENGINEERING.md` |
| `qa` | QA 验收 | `docs/QA.md` |
| `brainstorm` | 立项前讨论 | `docs/archive/raw/discussions/{模块}/{date}-*.md` 或 legacy `docs/讨论/{模块}/` |
| `bugfix` | bug 修复记录 | `docs/bugfix/{date}.md` / `docs/bugfix/backlog.md` |
| `research` | 一次性调研 | `docs/archive/raw/research/{date}-*.md` 或 legacy `docs/调研/{date}-*.md` |
| `plan` | 跨轮执行计划 | `docs/plans/{date}-*.md` |
| `guide` | 操作指南 | `docs/配置指南.md` / `docs/产品实现速查.md` / 当前 ops runbook |
| `reference` | 参考资料 | `docs/README.md` / `docs/INDEX.md` / `docs/modules/*.md` |
| `changelog` | 历史账本 | `docs/CHANGELOG.md` / `docs/*-CHANGELOG.md` |
...
| `module` 是 `global` 但路径在模块目录中 | ⚠️ 警告（应该用具体模块名） |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.2 | 2026-06-09 | 增加 `changelog`，适配当前真相源、active 工作区和 archive/raw |
